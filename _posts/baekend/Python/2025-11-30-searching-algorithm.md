...
**특징**
* `graph[i][j]`가 1이면 노드 i와 j가 연결되어 있음.
* 연결되지 않은 노드끼리는 무한의 비용(INF = 999999999) 또는 0
* 가중치가 있는 경우 해당 가중치 값을 저장
...
# 노드 1과 2는 연결되지 않음
print(graph[1][2]) # 999999999 (INF)
```

> **인접 행렬의 장점**
> 
> 1. **두 노드의 연결 여부를 $$O(1)$$에 확인 가능**
>   * **의미**: 어떤 두 정점 $$\mathit{u}$$와 $$\mathit{v}$$ 사이에 **간선(Edge)**이 존재하는지, 즉 두 노드가 직접 연결되어 있는지를 **상수 시간($$\mathbf{O(1)}$$)**만에 즉시 알 수 있다는 의미이다.
>   * **원리**: 인접 행렬은 $$\mathbf{V x V}$$ 크기의 2차원 배열 $$\mathbf{A}$$로 표현된다. $$A[u][v]$$의 값만 확인하면 되므로, 배열 인덱스 접근은 단 한 번의 연산으로 끝나기 때문에 $$\mathbf{O(1)}$$의 시간이 소요된다.
> 2. **구현이 간단함**
>   * **의미**: 그래프를 2차원 배열로 표현하기 때문에 코딩하기가 매우 쉽다.
>   * **원리**: 복잡한 포인터 연결이나 동적 메모리 할당 없이(파이썬은 노드 구조체 + 포인터 연결 및 메모리를 지정할 필요 없음(자동 관리)), 크기가 고정된 배열 하나만 선언하고 0 또는 1(가중치가 있다면 해당 가중치)로 채우면 된다.
> 3. 간선 추가/제거가 쉬움
>   * **의미**: 그래프에 새로운 간선을 추가하거나 기존 간선을 제거하는 작업이 매우 빠르고 간단합니다.
>   * **원리**:
>     * **추가**: 두 정점 $$\mathit{u}$$와 $$\mathit{v}$$를 연결하는 간선을 추가하려면 배열 **$$\mathbf{A[u][v]}$$의 값을 1로 설정하는 O(1)연산**만 수행하면 된다.
>     * **제거**: 간선을 제거하려면 **$$\mathbf{A[u][v]}$$의 값을 0으로 설정하는 O(1) 연산**만 수행하면 된다,

> **인접 행렬의 단점**        
> 
> 1. 항상 $$O(V^2)$$의 공간 필요
>   * **의미**: 그래프의 크기(정점의 개수 $$\mathit{V}$$)와 관계없이, 메모리 공간이 항상 **$$\mathit{V}$$의 제곱**에 비례하여 필요하다는 뜻
>   * **원리**: 인접 행렬은 $$\mathit{V}$$ x $$\mathit{V}$$ 크기의 2차원 배열을 사용하기 때문에 $$\mathit{V^2}$$개의 요소를 저장할 공간이 필요하다. 예를 들어, 정점이 10개($$\mathit{V}$$ = 10)이면 100개의 공간이 필요하다.
> 2. 모든 간선을 확인하려면 $$O(V^2)$$ 시간 필요
>   * **의미**: 그래프에 존재하는 모든 간선을 하나도 빠짐없이 검사하려면 **$$\mathit{V}$$의 제곱**에 비례하는 시간이 걸린다.
>   * **참고**: 만약 간선의 개수 $$\mathit{E}$$가 $$\mathit{V}$$에 비해 훨씬 적다면, 모든 간선을 확인하는 데  $$\mathbf{O(E)}$$ 시간이 걸리는 **인접 리스트**방식에 비해 비효율적입니다.
> 3. 희소 그래프(sparse graph)에서 메모리 낭비
>   * **의미**: 그래프에 비해 **간선(Edge)**의 개수가 **매우 적을 때** 인접 행렬을 사용하면 실제 간선이 없는 칸에도 메모리 공간을 할당하게 되어 낭비가 발생한다.
>   * **원리**:
>     * **밀집 그래프(Dense Graph)**:
