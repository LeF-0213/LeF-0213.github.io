...
related_posts:
    - /frontend/python
title:  "[Python] 백트래킹 알고리즘(Backtracking Algorithm)"
date:   2025-10-13
categories:
  - frontend
  - python
...
> **백트래킹(Backtracking)** 알고리즘은 해(solution)을 찾는 탐색 전략이다. **DFS(깊이 우선 탐색)**를 기반으로 하며, 현재 탐색 경로가 **해가 될 가능성이 없는 경우**에는 더 이상 진행하지 않고 **이전 상태로 되돌아가서(Backtrack)** 다른 경로를 탐색하는 기법이다.

→ 즉, 가능성이 없는 곳에서는 되돌아가고, 가능성이 있는 곳을 탐색하는 알고리즘
...
> 완전 탐색하는 방법보다 **가지치기(Pruning)**를 통해 탐색 시간을 획기적으로 줄여 **효율적**이다.     
> **시간 복잡도**
