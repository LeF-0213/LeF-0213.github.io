...
related_posts:
  - /aiLog/AIPractice
title:  "GPS 아트 경로 생성 트러블 슈팅 - Week 1 (1/28~2/2)"
date:   2026-01-28
categories:
  - aiPratice
description: >
...
문득 유사도를 높이고 속도를 높이기 위해서는 매번 유사도를 계산하는 것보다 처음부터 그림과 가까운 노드를 선택하는 건 어떨까란 생각이 들었음.
...
경로 상의 한 점이 출발지가 되게 하면 그림 유사도가 더 높아질 것 같아 코드를 수정해 보았다.

![Image](https://github.com/user-attachments/assets/681ed8c2-9831-4e3d-b660-6bf90a655a9e)

하지만 n_samples 중에 출발지가 가장 좋은 경로로 측정되어 달라지는 점이 없었다. 왜일까?

그림의 문제인가 싶어 svg_path를 하트에서 고구마 형태로 바꿔보았다.

![Image](https://github.com/user-attachments/assets/0cd1a2cf-c74b-4f0b-9067-cb38d74e4760)

**svg_path 즉, 원본 이미지 자체를 바꾸어 이미지를 바꾸어보았지만 그림의 시작점에서부터 경로가 그려진다.**

### 검증

경로 상의 한 점 중 정말로 출발지가 가장 유사도 높은 경로가 나오는 것인지 코드를 작성해 찍어보았다.

<img width="100%" alt="Image" src="https://github.com/user-attachments/assets/a448cee8-43ee-4720-b742-24277f4f1af4" />

출발지 (i_closest)가 가장 유사도 높은 경로로 전부 찍히고 있었다.

### 원인 분석

#### 왜 이런 결과가 나온 것일까?

아마도 원본 그림 자체가 출발지를 기준으로 회전과 스케일링이 적용되기 때문에, 항상 "출발지를 중심에 두는" 경로로 그려진다.

또한 유사도를 계산하는 함수는 "그림의 각 세그먼트 ↔︎ 경로 위 점" 거리로 계산되는데, 경로를 출발지 (i_closest)에서부터 시작하면 그림과의 위상이 맞아서 점수가 가장 좋아지는 것이었다.

> 즉:         
> - "출발지를 정한 상태에서 그림 경로가 그려져 있어서"      
> - 그림 상의 임의의 한 경로가 아니라, 출발지에 묶인 경로라서
> `best k(가장 유사도 높은 경로 상의 한 점)`가 항상 출발지 노드가 되어버린 것이다.

### 해결 방안

**문득 든 생각**: "그림 자체를 출발지에서부터 시작하지 않고 그림 선분 상의 한 점이 출발지가 되게 하면?"

그림을 "출발지에서 시작"하지 않고, 그림 선분 위 한 점만 출발지에 맞추는 방식으로 코드를 수정하고 테스트를 진행해야겠다.

#### 바꾸는 방향: "그림은 그대로, 선분 위 한 점만 출발지"

1. **회전・스케일은 그림 자체 기준으로만 함**
  - 회전 중심: 출발지 말고 그림의 중심
  - 스케일: 목표 거리에 맞추되, 그림 전체 길이/범위 기준
  - 그림의 "첫점 -> ... -> 끝 점" 순서로 적용
2. **그 다음에 "선분 위 한 점을 출발지에 맞추기**
  - 스케일・회전된 그림에서 출발지와 가장 가까운 점을 찾음 -> 그게 "그림 선분 상의 출발지"
  - 필요하면 전체 그림을 평행 이동해서, 그 점이 출발지 좌표와 일치하도록 함
  - 출발지는 "그림의 시작점" 이 아니라 "그림 위 한 점"이 됨

**이렇게 하면**:
- 그림은 출발지에서 시작하지 않고, SVG 순서대로(예: 하트 꼭지점부터) 시작
- 그림 선분상 한 점만 출발지 위치에 오게 하고
- waypoint/i_closest 로직은 그대로 두어도 "출발지 = 경로 위 한 점"이 자연스럽게 유지될 것으로 예상.

### 구현 시도 및 문제점

<img width="100%" alt="Image" src="https://github.com/user-attachments/assets/46f1e235-75b7-4ed8-b9f7-f0a7f13f3563" />

그림이 중심을 기준으로 회전하고 그 뒤 출발지를 정해 경로가 이상하게 생성되었다.

#### 문제 원인

살펴보니 현재 `회전 -> 스케일링 -> A* -> 유사도 평가`를 진행하고 있어 회전과 스케일링 자체가 그림의 중심점을 출발지로 두고 이루어진 후 경로 생성하고 있어 원하는 방향과 다르게 그려지는 중이었다.

#### 해결 방안

`선분 위의 후보점 배치 조사: k번째 점이 출발지에 오도록 평행 이동만 -> 각 배치마다 회전 -> 스케일 -> 유사도 평가` 순서로 진행해 원래 의도대로 더 다양한 결과가 나올 수 있게 코드를 수정한다.

### 검증

<img width="100%" alt="Image" src="https://github.com/user-attachments/assets/2da49246-c4d6-435c-be03-5a3763e9dc4a" />

바뀐 건지 잘 모르겠어서 원본 그림 자체를 바꿔보았다.

<img width="100%" alt="Image" src="https://github.com/user-attachments/assets/156b52a2-bbb8-495c-9239-2d11340ce2ff" />

> 원본 그림 상 출발하는 곳이 전부 달라졌다. 경로 상의 한 점이 출발지가 되게 **잘 바뀐 것 같다.**      

문제는 속도가 70초대로 늘어났는데, 유사도를 높이는 것이 목표였기에 이 방식을 그대로 가져갈 것 같다.

다만, 최적화를 하기 전 실제 프로젝트에 테스트 한 함수를 백에 적용하고, 프론트와 연결하는 게 우선이라 그것부터 진행한 후에 해야할 것 같다.
